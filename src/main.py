...
=======
    dfs_dir_directa = gf.leer_excel_directa(
...
if __name__ == "__main__":
    ...
    def pares_vnaa(
        ...
        for c in vcols:
            ...
            if c.endswith(f" {token_a}") or re.search(
                rf" {re.escape(token_a)}\.\d+$", c
            ):
                A[idx(c)] = c
            elif c.endswith(f" {token_b}") or re.search(
                rf" {re.escape(token_b)}\.\d+$", c
            ):
                B[idx(c)] = c
